...
export const SetupScreen: React.FC<SetupScreenProps> = ({ onComplete, onBack }) => {
  ...
  const [placement, setPlacement] = useState('2F');
  const [position, setPosition] = useState('主任');
  ...
  const positions = ['科長', '係長', '主査', '主任', '主事', '会計年度', 'その他'];
  ...
  const handleRoleToggle = (role: string) => {
    ...
    // If privileged role is chosen, check if it's already selected
    if (selectedRoles.includes(role)) {
      ...
      // Attempt to toggle on -> Need password
      setPendingRole(role);
  ...
  const verifyPassword = () => {
    if (passwordInput === ADMIN_PASSWORD) {
      if (pendingRole) {
        setSelectedRoles(prev => {
          const next = [...prev.filter(r => r !== '一般職員'), pendingRole];
          return next;
        });
      }
  ...
  const handleFinish = () => {
    ...
    try {
      ...
      const newStaff = {
        ...
        isApproved: isYoshidaAdmin, // Yoshida (Admin) is auto-approved
        updatedAt: new Date().toISOString()
      };
      
      if (Platform.OS === 'web') {
        if (isYoshidaAdmin) {
          alert('登録完了: 管理者として承認されました。そのままログイン可能です。');
        } else {
          alert('登録申請を送信しました。\n\n管理者が内容を確認し、承認するまでログインはできません。しばらくお待ちください。');
        }
        onComplete(newStaff);
      } else {
        if (isYoshidaAdmin) {
          Alert.alert(
            '登録完了', 
            '管理者として承認されました。',
            [{ text: '了解', onPress: () => onComplete(newStaff) }]
          );
        } else {
          Alert.alert(
            '登録申請を受け付けました', 
            '管理者の承認待ちです。\n\n承認されるまでログインはできませんので、管理者にご確認ください。',
            [{ 
              text: '了解', 
              onPress: () => onComplete(newStaff) 
            }]
          );
        }
      }
    } catch (e: any) {
      Alert.alert('エラー', '完了処理中にエラーが発生しました: ' + e.message);
    }
  };
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll}>
        <View style={styles.header}>
          ...
          <ThemeText variant="body" color={COLORS.textSecondary}>最初の設定を行ってください</ThemeText>
        </View>

        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <User size={18} color={COLORS.primary} />
            <ThemeText variant="body" bold style={{ marginLeft: 8 }}>お名前</ThemeText>
          </View>
          <TextInput
            style={styles.input}
            placeholder="例: 山田 太郎"
      ...
      <Modal visible={isPasswordModalVisible} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <ThemeCard style={styles.passwordCard}>
            <ThemeText variant="h2" style={{ marginBottom: 12, textAlign: 'center' }}>
              権限の認証
            </ThemeText>
            <ThemeText variant="body" color={COLORS.textSecondary} style={{ marginBottom: 20, textAlign: 'center' }}>
              「{pendingRole}」権限を選択するにはパスワードが必要です
            </ThemeText>
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity 
                ...
                onPress={() => { setIsPasswordModalVisible(false); setPendingRole(null); }}
              >
                <ThemeText color={COLORS.textSecondary}>キャンセル</ThemeText>
              </TouchableOpacity>
              <TouchableOpacity style={styles.confirmBtn} onPress={verifyPassword}>
                <ThemeText color="white" bold>認証</ThemeText>
              </TouchableOpacity>
            </View>
          </ThemeCard>
        </View>
...
const styles = StyleSheet.create({
  ...
  input: {
    ...
    color: COLORS.text,
    fontSize: 16,
    // Removed borders/outlines for cleaner look
    // @ts-ignore - Web only to remove focus ring
    outlineWidth: 0,
  },
