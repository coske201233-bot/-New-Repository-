...
const backupPath = 'current_requests_model.json';
const backup = JSON.parse(fs.readFileSync(backupPath, 'utf8'));

async function restoreJune() {
  console.log('--- RESTORING JUNE FROM MODEL FILE ---');
  
  // 1. Wipe existing June records
  console.log('Wiping existing June records (2026-06)...');
  ...
  console.log(`Found ${toRestore.length} records in ${backupPath} to restore.`);
